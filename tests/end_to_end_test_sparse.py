...
NOISER = hs.noiser.sparse.Sparse
...
frozen_params, params, scan_map, es_map = MODEL.rand_init(model_key, in_dim=in_dim, out_dim=1, hidden_dims=[512], use_bias=True, activation="relu", dtype="float32")
...
for epoch in range(num_epochs):
    ...
    validations_results.append(jnp.mean(raw_validation_scores))
    print(f"Avg validation score is {validations_results[-1]}")
    outputs_batch = jit_forward(noiser_params, params, iterinfo, input_batch)
    raw_scores = batch_calculate_fitnesses(outputs_batch)
    results.append(jnp.mean(raw_scores))
    # print(f"Avg training score is {results[-1]}")
...
plt.plot(validations_results, label="validation")
plt.legend()
plt.show()