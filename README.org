...
WARNING: This codebase is a research preview. We strongly recommend reading through the entire src directory and the example in tests/end_to_end_test.py to understand the codebase. We recommend also looking at the [[https://github.com/ESHyperscale/nano-egg][nano-egg]] repository, which contains a single-file implementation of int8 pretraining of a language model from scratch.
...
The two core components of the codebase are the Noiser (src/hyperscalees/noiser) and the Model (src/hyperscalees/models).
